		...
		methods: {
			...
			async sureBtn(){//确认输入
				...
				if(!that.formItem.BuyName || that.formItem.BuyName == ''){
					pullToast('请输入购买人姓名！',1500,'warning');
					return false;
				}
				//pullLoading('('加载中···');
			    let res = await that.$http.ReqPOST('User/CheckScanCodeForRetail',{ScanCode:that.formItem.scan,BuyName:that.formItem.BuyName});
			...
			async submit(){//确认提交
				...
				//pullLoading('('条码提交中···');
				let res = await this.$http.ReqPOST('User/InsertGoodsByScanCodeByRetail',{BuyName:this.formItem.BuyName,scanCodeJson:JSON.stringify(this.scanData)});
				pullToast('操作成功');
				this.clear();
			},
			clear(){//清除
