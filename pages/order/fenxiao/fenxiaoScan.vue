	...
	export default {
		...
		methods: {
			...
			async sureBtn(){//确认输入
				...
				// //pullLoading('('加载中···')
			    let res = await this.$http.ReqPOST('User/CheckScanCodeForUndertakes',{ScanCode:that.formItem.scan,UserCode:that.formItem.dealerId});
			...
			async submit(){//确认提交
				...
				let res = await this.$http.ReqPOST('User/InsertGoodsByScanCodeByUndertakes',{scanCodeJson:JSON.stringify(this.scanData),UserCode:this.formItem.dealerId});
				pullToast('操作成功');
				this.clear();
			},
			clear(){//清除
