	...
	export default {
		...
		methods: {
			...
			async sureBtn(){//确认输入
				...
				//pullLoading('('加载中···')
			    let res = await this.$http.ReqPOST('User/CheckScanCodeForUsedself',{ScanCode:that.formItem.scan});
			...
			async submit(){//确认提交
				...
				//pullLoading('('条码提交中···');
				let res = await this.$http.ReqPOST('User/InsertGoodsByScanCodeByUsedself',{scanCodeJson:JSON.stringify(this.scanData)});
				pullToast('操作成功');
				this.clear();
			},
			clear(){//清除
